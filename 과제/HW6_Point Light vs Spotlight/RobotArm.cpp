...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        PhongShader->setMat4("projection", glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f));//vs
        PhongShader->setMat4("view", camera.GetViewMatrix());//vs
        PhongShader->setVec3("viewPos", camera.Position);//fs

        PhongShader->setVec3("light.position", camera.Position);//fs
        ...
        FloorShader->setMat4("projection", glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f));//vs
        FloorShader->setMat4("view", camera.GetViewMatrix());//vs
        FloorShader->setVec3("viewPos", camera.Position);//fs


        FloorShader->setVec3("light.position", camera.Position);//fs
        ...
        std::cout << (spot ? "SpotLight" : "PointLight") << std::endl;
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS && !spotKeyPressed)
    {
        spot = !spot;
        spotKeyPressed = true;
    }
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    float xoffset_screen = (float)(xpos - lastX);
    float yoffset_screen = (float)(lastY - ypos);

    lastX = (float)xpos;
    lastY = (float)ypos;

    if (RightButtonDown) {
        camera.ProcessMouseMovement(xoffset_screen, yoffset_screen);
    }
